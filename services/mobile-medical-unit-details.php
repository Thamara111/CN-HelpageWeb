...
<div class="max-w-7xl mx-auto px-4 py-12 grid grid-cols-1 lg:grid-cols-3 gap-8">
    ...
    <!-- Main Content -->
    <div class="lg:col-span-2" data-aos="fade-left" data-aos-duration="1500">
        <div id="mmu-container"></div>
...
<script>
    $(document).ready(async function () {
        ...
        const programId = params.get("mmu");

        if (!programId) {
            $("#mmu-container").html(
                '<p class="text-red-500 text-center">Invalid program request.</p>'
            );
            return;
        }
        ...
        try {
            ...
            // Render main program
            if (program) {
                $("#mmu-container").html(renderProgramHTML(program));
            } else {
                $("#mmu-container").html(
