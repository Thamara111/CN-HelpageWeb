...
<!-- Contact Section -->
<section id="health-donations-container" class="">
    <div class="container mx-auto px-6 py-12">
        ...
        <section class="w-full px-4 md:px-8  bg-white">
            <div id="day-center-container" class="grid grid-cols-1 sm:grid-cols-2  gap-8">
            </div>
...
<script>
    $(document).ready(function () {
        const container = $("#day-center-container");
        $.getJSON('/assets/json/services.json', function (data) {

            // Filter only "Day Center" category
            const dayCenterData = data.Services
                .filter(cat => cat.category === "Day Center")
                .flatMap(cat => cat.services_list); // flatten the programs array

            // Loop through filtered programs
            $.each(dayCenterData, function (index, item) {
                const card = `
            ...
            console.error("Failed to load Day Center Services.");
            $('#loading-state').html('<p class="text-red-500">Failed to load Day Center Services. Please try again later.</p>');
        });
    });
